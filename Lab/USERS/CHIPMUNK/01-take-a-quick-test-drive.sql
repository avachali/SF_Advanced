...
CREATE SCHEMA if not exists CHIPMUNK_db.myschema;
CREATE or replace  WAREHOUSE CHIPMUNK_wh INITIALLY_SUSPENDED=TRUE AUTO_SUSPEND=300;
CREATE or replace WAREHOUSE CHIPMUNK_wh_small WAREHOUSE_SIZE=SMALL INITIALLY_SUSPENDED=TRUE AUTO_SUSPEND=300;
CREATE or replace WAREHOUSE CHIPMUNK_wh_large WAREHOUSE_SIZE=LARGE INITIALLY_SUSPENDED=TRUE AUTO_SUSPEND=300;

-- AV
CREATE TRANSIENT DATABASE if not exists CHIPMUNK_Transient_Db;
use database CHIPMUNK_Transient_Db;
CREATE temporary TABLE "CHIPMUNK_TRANSIENT_DB"."PUBLIC"."TEST" ("C1" STRING) COMMENT = 'test';
-- End AV
...
USE ROLE TRAINING_ROLE;
USE WAREHOUSE CHIPMUNK_wh_small;
...
ALTER USER CHIPMUNK
    ...
    DEFAULT_NAMESPACE=CHIPMUNK_DB.PUBLIC
    DEFAULT_WAREHOUSE=CHIPMUNK_wh_small;
