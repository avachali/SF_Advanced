...
CREATE WAREHOUSE IF NOT EXISTS CHIPMUNK_LOAD_WH
...
--         First, click on your database CHIPMUNK_DB in the navigator. Locate and
...
USE ROLE training_role;
USE SCHEMA CHIPMUNK_db.public;
USE WAREHOUSE CHIPMUNK_load_wh;


-- 6.2.2   Create a named stage:

CREATE  OR REPLACE TEMPORARY STAGE CHIPMUNK_STAGE;


-- 6.2.3   Change your warehouse size to small:

ALTER WAREHOUSE CHIPMUNK_load_wh SET WAREHOUSE_SIZE = SMALL;


-- 6.2.4   Download a file from the Citibike table to the stage you created:

COPY INTO @CHIPMUNK_STAGE/citibike/singlefile/citibike.tbl
...
ls @CHIPMUNK_STAGE/citibike/singlefile;
...
COPY INTO @CHIPMUNK_STAGE/citibike/multiplefiles/citibike_
  FROM (SELECT * FROM citibike.schema1.trips)
  FILE_FORMAT=(FORMAT_NAME=training_db.traininglab.MYPIPEFORMAT)
  SINGLE=FALSE;

ls @CHIPMUNK_STAGE/citibike/multiplefiles;
...
-- 6.3.7   Alter the CHIPMUNK_load_wh and increase its size:

ALTER WAREHOUSE CHIPMUNK_LOAD_WH SET WAREHOUSE_SIZE='X-LARGE';
...
COPY INTO CHIPMUNK_DB.PUBLIC.CUSTOMER 
...
--         INTO statements to load each table in your CHIPMUNK_DB.PUBLIC schema.
--         Remaining tables:
...
-- 6.4.1   Empty the REGION Table in the PUBLIC schema of your CHIPMUNK_DB:
...
CREATE FILE FORMAT CHIPMUNK_FIXED TYPE = 'CSV' 
                   COMPRESSION = 'AUTO' 
...
COPY INTO nation_tbl
  FROM (SELECT CAST(SUBSTR($1,1,2) AS NUMBER)
        ...
        FROM '@training_db.traininglab.ed_stage/coredata/TCPH/FIXFORMAT'
          ...
          FILE_FORMAT=(FORMAT_NAME=CHIPMUNK_FIXED);
...
-- 6.9.6   Select CHIPMUNK_DB Database and confirm that the REGION table has data
--         loaded.
