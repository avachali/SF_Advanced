...
CREATE OR REPLACE PIPE CHIPMUNK_db.citibike.trips_pipe  AS
  COPY INTO CHIPMUNK_db.citibike.trips
    FROM (SELECT *
    FROM @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/CHIPMUNK)
...
COPY INTO @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/CHIPMUNK/citibike1_
...
LS @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/CHIPMUNK;
...
ALTER PIPE CHIPMUNK_db.citibike.trips_pipe REFRESH;

SELECT system$pipe_status('CHIPMUNK_db.citibike.trips_pipe');
...
COPY INTO @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/CHIPMUNK/citibike2_
  FROM  (SELECT *  FROM CITIBIKE.SCHEMA1.TRIPS SAMPLE(10 ROWS))
   FILE_FORMAT = (FORMAT_NAME = training_db.traininglab.MYGZIPPIPEFORMAT)
   OVERWRITE = TRUE;
