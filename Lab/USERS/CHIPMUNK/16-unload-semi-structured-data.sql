...
create or replace stage CHIPMUNK_unload;
...
--         JSON files in the CHIPMUNK_unload stage, with a max file size of 10MB.
...
copy into @CHIPMUNK_unload/json/weather/2019/US/stations from 
(
  select v from TRAINING_DB.WEATHER.ISD_2019_TOTAL
...
list @CHIPMUNK_unload/json/weather/2019/US/;
...
CREATE OR REPLACE PROCEDURE UNLOAD_ISD_2019_TOTAL(COUNTRY_CODE STRING)
    ...
    try {
      var unload_query = "copy into @CHIPMUNK_unload/json/weather/2019/" +  
                          COUNTRY_CODE + 
                         "/stations from" +
                         "( select v from TRAINING_DB.WEATHER.ISD_2019_TOTAL" +
...
copy into @CHIPMUNK_unload/json/customers from 
(
  select object_construct(
          'customer_info',object_construct(
...
copy into @CHIPMUNK_unload/parquet/weather/2019/US/stations from 
(
  SELECT weather.t as date, 
       v:station.name::STRING AS station,
       v:station.country::STRING AS country,
...
list @CHIPMUNK_unload/parquet/weather/2019/US/stations;


-- 16.4.6  Query the PARQUET file in the stage.

select $1 from @CHIPMUNK_unload/parquet/weather/2019/US/stations_0_0_0.snappy.parquet (file_format => parquetformat);
