...
DESCRIBE NETWORK POLICY CHIPMUNK_policy;


-- 4.1.4   Now, create a new user:

CREATE USER CHIPMUNK_testuser PASSWORD = 'Password@1' MUST_CHANGE_PASSWORD = FALSE;


-- 4.1.5   Log out of the Snowflake account, and log back in as the user
--         CHIPMUNK_testuser you just created. This is just to verify that the
--         new user can log in. After you have done that, log back out and log
--         in as yourself CHIPMUNK.
...
--         CHIPMUNK_testuser. You will not be able to. Then log in as yourself
--         CHIPMUNK. Since the network policy has only been applied to the test
--         user, it does not impact your ability to access the Snowflake
--         account.

-- 4.1.8   Drop the policy and user you created:

DROP USER CHIPMUNK_testuser;
DROP NETWORK POLICY CHIPMUNK_policy;
...
ALTER SCHEMA CHIPMUNK_db.public SET DATA_RETENTION_TIME_IN_DAYS = 10;
...
ALTER DATABASE CHIPMUNK_db SET DATA_RETENTION_TIME_IN_DAYS = 2;
...
ALTER SCHEMA CHIPMUNK_db.public UNSET DATA_RETENTION_TIME_IN_DAYS;
