...
CREATE TEMPORARY TABLE INSTRUCTOR2_db.public.lineitem_training 
  AS SELECT * FROM  lineitem SAMPLE (90);

CREATE TEMPORARY TABLE INSTRUCTOR2_db.public.lineitem_testing
  AS SELECT * FROM lineitem
      WHERE l_orderkey NOT IN (SELECT l_orderkey
  FROM INSTRUCTOR2_db.public.lineitem_training);
...
ALTER WAREHOUSE INSTRUCTOR2_query_wh
...
USE WAREHOUSE INSTRUCTOR2_QUERY_WH;
ALTER WAREHOUSE INSTRUCTOR2_QUERY_WH SET WAREHOUSE_SIZE = 'XSMALL';
...
USE ROLE training_role;
CREATE SCHEMA INSTRUCTOR2_db.hierarchy;
USE SCHEMA INSTRUCTOR2_db.hierarchy;
USE WAREHOUSE INSTRUCTOR2_query_wh;
