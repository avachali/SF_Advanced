...
CREATE WAREHOUSE IF NOT EXISTS INSTRUCTOR2_QUERY_WH;
USE WAREHOUSE INSTRUCTOR2_QUERY_WH;
CREATE DATABASE INSTRUCTOR2_SHARE_DB;
USE DATABASE INSTRUCTOR2_SHARE_DB;
-- PROVIDER --
...
-- PROVIDER --
CREATE SHARE INSTRUCTOR2_SHARE;
-- PROVIDER --
...
-- PROVIDER --
GRANT USAGE ON DATABASE INSTRUCTOR2_SHARE_DB TO SHARE INSTRUCTOR2_SHARE;

GRANT USAGE ON SCHEMA INSTRUCTOR2_SHARE_DB.DS_TPCH_SF1 TO SHARE INSTRUCTOR2_SHARE;

GRANT SELECT ON TABLE INSTRUCTOR2_SHARE_DB.DS_TPCH_SF1.CUSTOMER TO SHARE INSTRUCTOR2_SHARE;

GRANT SELECT ON TABLE INSTRUCTOR2_SHARE_DB.DS_TPCH_SF1.ORDERS TO SHARE INSTRUCTOR2_SHARE;
-- PROVIDER --
...
-- PROVIDER --
ALTER SHARE INSTRUCTOR2_SHARE SET ACCOUNTS=[consumer-account];
-- PROVIDER --
...
-- PROVIDER --
SHOW GRANTS TO SHARE INSTRUCTOR2_SHARE;
-- PROVIDER --
...
CREATE OR REPLACE WAREHOUSE INSTRUCTOR2_QUERY_WH 
...
-- CONSUMER --
SHOW SHARES LIKE 'INSTRUCTOR2_SHARE';

DESCRIBE SHARE [provider-account].INSTRUCTOR2_SHARE;
-- CONSUMER --
...
-- CONSUMER --
CREATE DATABASE INSTRUCTOR2_DS_CONSUMER
FROM SHARE [provider-account].INSTRUCTOR2_SHARE;
USE DATABASE INSTRUCTOR2_DS_CONSUMER;
...
-- CONSUMER --
GRANT USAGE ON WAREHOUSE INSTRUCTOR2_query_wh TO ROLE public;
...
USE ROLE TRAINING_ROLE;
GRANT USAGE ON DATABASE INSTRUCTOR2_DS_CONSUMER TO ROLE public;
--You should have received an error message: you cannot add privileges to a share

GRANT IMPORTED PRIVILEGES ON DATABASE INSTRUCTOR2_DS_CONSUMER TO ROLE PUBLIC;
...
-- PROVIDER --
GRANT SELECT ON INSTRUCTOR2_SHARE_DB.DS_TPCH_SF1.CUST_SENSITIVE_DATA_VW TO SHARE INSTRUCTOR2_SHARE;
...
-- CONSUMER --
USE ROLE TRAINING_ROLE;
CREATE OR REPLACE WAREHOUSE INSTRUCTOR2_LOAD_WH 
...
GRANT IMPORTED PRIVILEGES ON DATABASE INSTRUCTOR2_DS_CONSUMER TO ROLE TRAINING_ROLE;
-- CONSUMER --
...
-- CONSUMER --
SHOW DATABASES LIKE 'INSTRUCTOR2_DS_CONSUMER';
SHOW SCHEMAS IN DATABASE INSTRUCTOR2_DS_CONSUMER;
SHOW TABLES IN SCHEMA INSTRUCTOR2_DS_CONSUMER.DS_TPCH_SF1;
SHOW VIEWS IN SCHEMA INSTRUCTOR2_DS_CONSUMER.DS_TPCH_SF1;
...
-- CONSUMER --
SELECT COUNT(*)
FROM INSTRUCTOR2_DS_CONSUMER.DS_TPCH_SF1.CUST_SENSITIVE_DATA_VW;
-- expected row count =  20
SELECT COUNT(*)
FROM INSTRUCTOR2_DS_CONSUMER.DS_TPCH_SF1.CUSTOMER;
-- expected row count = 150000
SELECT COUNT(*)
FROM INSTRUCTOR2_DS_CONSUMER.DS_TPCH_SF1.ORDERS;
...
-- PROVIDER --
USE INSTRUCTOR2_SHARE_DB;
DESCRIBE SHARE INSTRUCTOR2_SHARE;
-- Revoke access to the ORDERS table:
REVOKE SELECT ON TABLE INSTRUCTOR2_SHARE_DB.DS_TPCH_SF1.ORDERS FROM SHARE INSTRUCTOR2_SHARE;
-- PROVIDER --
...
-- CONSUMER --
DESC SHARE [provider-account].INSTRUCTOR2_SHARE;
SELECT COUNT (*) FROM INSTRUCTOR2_DS_CONSUMER.DS_TPCH_SF1.ORDERS;
...
-- PROVIDER --
DELETE
  FROM INSTRUCTOR2_SHARE_DB.DS_TPCH_SF1.CUSTOMER
  WHERE C_CUSTKEY = 1;
...
-- PROVIDER --
CREATE DATABASE IF NOT EXISTS INSTRUCTOR2_UDF_DEMO;
CREATE SCHEMA IF NOT EXISTS INSTRUCTOR2_UDF_DEMO.PUBLIC;

CREATE OR REPLACE TABLE INSTRUCTOR2_udf_demo.public.sales AS
...
select 6139 as input_item
  ...
  from INSTRUCTOR2_udf_demo.public.sales  
...
-- PROVIDER --
CREATE OR REPLACE SECURE FUNCTION INSTRUCTOR2_UDF_DEMO.PUBLIC.GET_MARKET_BASKET(INPUT_ITEM_SK NUMBER(38))
...
AS
 'SELECT INPUT_ITEM_SK
       ...
       , COUNT(DISTINCT SS_TICKET_NUMBER) BASKETS
    FROM INSTRUCTOR2_UDF_DEMO.PUBLIC.SALES
    WHERE SS_TICKET_NUMBER IN (SELECT SS_TICKET_NUMBER FROM INSTRUCTOR2_UDF_DEMO.PUBLIC.SALES WHERE SS_ITEM_SK = INPUT_ITEM_SK)
GROUP BY SS_ITEM_SK
ORDER BY 3 DESC, 2';

SELECT * FROM TABLE(INSTRUCTOR2_UDF_DEMO.PUBLIC.GET_MARKET_BASKET(6139));
-- PROVIDER --
...
-- PROVIDER --
USE DATABASE INSTRUCTOR2_udf_demo;
CREATE SHARE IF NOT EXISTS INSTRUCTOR2_UDF_DEMO_SHARE;
GRANT USAGE ON DATABASE INSTRUCTOR2_UDF_DEMO TO SHARE INSTRUCTOR2_UDF_DEMO_SHARE;
GRANT USAGE ON SCHEMA INSTRUCTOR2_UDF_DEMO.PUBLIC to share INSTRUCTOR2_UDF_DEMO_SHARE;
GRANT USAGE ON FUNCTION INSTRUCTOR2_UDF_DEMO.PUBLIC.get_market_basket(number) to share INSTRUCTOR2_UDF_DEMO_SHARE;
ALTER SHARE INSTRUCTOR2_UDF_DEMO_SHARE ADD ACCOUNTS=[consumer-account];
-- PROVIDER --
...
-- CONSUMER --
USE ROLE TRAINING_ROLE;
CREATE DATABASE INSTRUCTOR2_UDF_TEST FROM SHARE [provider-account].INSTRUCTOR2_UDF_DEMO_SHARE;
GRANT IMPORTED PRIVILEGES ON DATABASE INSTRUCTOR2_UDF_TEST TO ROLE PUBLIC;
USE DATABASE INSTRUCTOR2_UDF_TEST;
SELECT * FROM TABLE(INSTRUCTOR2_UDF_TEST.PUBLIC.GET_MARKET_BASKET(6139));
-- CONSUMER --
...
-- PROVIDER --
DESCRIBE SHARE INSTRUCTOR2_UDF_DEMO_SHARE;
-- PROVIDER --
