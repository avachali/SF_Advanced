...
CREATE SCHEMA IF NOT EXISTS INSTRUCTOR2_db.CITIBIKE;
USE schema INSTRUCTOR2_db.CITIBIKE;
...
ALTER WAREHOUSE INSTRUCTOR2_load_wh SET WAREHOUSE_SIZE=MEDIUM;


-- 10.2.3  Copy data to the external stage.

COPY INTO '@TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike1'
...
ls @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike1;


-- 10.2.4  Set the warehouse size to LARGE for the second test.

ALTER WAREHOUSE INSTRUCTOR2_load_wh SUSPEND;
...
COPY INTO '@TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike2'
  FROM (SELECT * FROM CITIBIKE.SCHEMA1.TRIPS)
  FILE_FORMAT=(FORMAT_NAME=training_db.traininglab.MYGZIPPIPEFORMAT)
  MAX_FILE_SIZE=49000000
;

ls @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike2;
...
remove @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike1;

remove @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike2;


-- 10.2.8  Unload the CITIBIKE data into Parquet files.

COPY INTO '@TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike/trips'
  FROM (SELECT * FROM CITIBIKE.SCHEMA1.TRIPS)
  FILE_FORMAT=(FORMAT_NAME=training_db.traininglab.MYPARQUETFORMAT)
  MAX_FILE_SIZE=49000000;

ls @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike;
...
CREATE OR REPLACE EXTERNAL TABLE EXT_PARQUET_TRIPS
  location= @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/citibike/
...
COPY INTO '@TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/parquet/tpch/myjoin'
  FROM (SELECT c_name
     , c_nationkey
     , c_address
     , c_acctbal
...
CREATE OR REPLACE EXTERNAL TABLE EXT_PARQUET_myjoin
  location= @TRAINING_DB.TRAININGLAB.CLASS_STAGE/COURSE/ADVANCED/student/INSTRUCTOR2/parquet/tpch/
  FILE_FORMAT=(TYPE=PARQUET);
...
USE ROLE training_role;
USE WAREHOUSE INSTRUCTOR2_query_wh;
CREATE DATABASE INSTRUCTOR2_tpcdi_stg;
USE SCHEMA public;
...
CREATE OR REPLACE FILE FORMAT INSTRUCTOR2_TPCDI_STG.PUBLIC.TXT_FIXED_WIDTH 
  TYPE = CSV
  COMPRESSION = 'AUTO' 
