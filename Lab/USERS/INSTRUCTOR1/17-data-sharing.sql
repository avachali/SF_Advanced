...
CREATE DATABASE INSTRUCTOR1_SHARE_DB;
USE DATABASE INSTRUCTOR1_SHARE_DB;
-- PROVIDER --
...
-- PROVIDER --
CREATE SHARE INSTRUCTOR1_SHARE;
-- PROVIDER --
...
-- PROVIDER --
GRANT USAGE ON DATABASE INSTRUCTOR1_SHARE_DB TO SHARE INSTRUCTOR1_SHARE;

GRANT USAGE ON SCHEMA INSTRUCTOR1_SHARE_DB.DS_TPCH_SF1 TO SHARE INSTRUCTOR1_SHARE;

GRANT SELECT ON TABLE INSTRUCTOR1_SHARE_DB.DS_TPCH_SF1.CUSTOMER TO SHARE INSTRUCTOR1_SHARE;

GRANT SELECT ON TABLE INSTRUCTOR1_SHARE_DB.DS_TPCH_SF1.ORDERS TO SHARE INSTRUCTOR1_SHARE;
-- PROVIDER --
...
-- PROVIDER --
ALTER SHARE INSTRUCTOR1_SHARE SET ACCOUNTS=[consumer-account];
-- PROVIDER --
...
-- PROVIDER --
SHOW GRANTS TO SHARE INSTRUCTOR1_SHARE;
-- PROVIDER --
...
CREATE OR REPLACE WAREHOUSE INSTRUCTOR1_QUERY_WH 
...
-- CONSUMER --
SHOW SHARES LIKE 'INSTRUCTOR1_SHARE';

DESCRIBE SHARE [provider-account].INSTRUCTOR1_SHARE;
-- CONSUMER --
...
-- CONSUMER --
CREATE DATABASE INSTRUCTOR1_DS_CONSUMER
FROM SHARE [provider-account].INSTRUCTOR1_SHARE;
USE DATABASE INSTRUCTOR1_DS_CONSUMER;
...
-- CONSUMER --
GRANT USAGE ON WAREHOUSE INSTRUCTOR1_query_wh TO ROLE public;
...
USE ROLE TRAINING_ROLE;
GRANT USAGE ON DATABASE INSTRUCTOR1_DS_CONSUMER TO ROLE public;
--You should have received an error message: you cannot add privileges to a share

GRANT IMPORTED PRIVILEGES ON DATABASE INSTRUCTOR1_DS_CONSUMER TO ROLE PUBLIC;
...
-- PROVIDER --
GRANT SELECT ON INSTRUCTOR1_SHARE_DB.DS_TPCH_SF1.CUST_SENSITIVE_DATA_VW TO SHARE INSTRUCTOR1_SHARE;
...
-- CONSUMER --
USE ROLE TRAINING_ROLE;
CREATE OR REPLACE WAREHOUSE INSTRUCTOR1_LOAD_WH 
...
GRANT IMPORTED PRIVILEGES ON DATABASE INSTRUCTOR1_DS_CONSUMER TO ROLE TRAINING_ROLE;
-- CONSUMER --
...
-- CONSUMER --
SHOW DATABASES LIKE 'INSTRUCTOR1_DS_CONSUMER';
SHOW SCHEMAS IN DATABASE INSTRUCTOR1_DS_CONSUMER;
SHOW TABLES IN SCHEMA INSTRUCTOR1_DS_CONSUMER.DS_TPCH_SF1;
SHOW VIEWS IN SCHEMA INSTRUCTOR1_DS_CONSUMER.DS_TPCH_SF1;
...
-- CONSUMER --
SELECT COUNT(*)
FROM INSTRUCTOR1_DS_CONSUMER.DS_TPCH_SF1.CUST_SENSITIVE_DATA_VW;
-- expected row count =  20
SELECT COUNT(*)
FROM INSTRUCTOR1_DS_CONSUMER.DS_TPCH_SF1.CUSTOMER;
-- expected row count = 150000
SELECT COUNT(*)
FROM INSTRUCTOR1_DS_CONSUMER.DS_TPCH_SF1.ORDERS;
...
-- PROVIDER --
USE INSTRUCTOR1_SHARE_DB;
DESCRIBE SHARE INSTRUCTOR1_SHARE;
-- Revoke access to the ORDERS table:
REVOKE SELECT ON TABLE INSTRUCTOR1_SHARE_DB.DS_TPCH_SF1.ORDERS FROM SHARE INSTRUCTOR1_SHARE;
-- PROVIDER --
...
-- CONSUMER --
DESC SHARE [provider-account].INSTRUCTOR1_SHARE;
SELECT COUNT (*) FROM INSTRUCTOR1_DS_CONSUMER.DS_TPCH_SF1.ORDERS;
...
-- PROVIDER --
DELETE
  FROM INSTRUCTOR1_SHARE_DB.DS_TPCH_SF1.CUSTOMER
  WHERE C_CUSTKEY = 1;
...
-- PROVIDER --
CREATE DATABASE IF NOT EXISTS INSTRUCTOR1_UDF_DEMO;
CREATE SCHEMA IF NOT EXISTS INSTRUCTOR1_UDF_DEMO.PUBLIC;

CREATE OR REPLACE TABLE INSTRUCTOR1_udf_demo.public.sales AS
...
select 6139 as input_item
  ...
  from INSTRUCTOR1_udf_demo.public.sales  
...
-- PROVIDER --
CREATE OR REPLACE SECURE FUNCTION INSTRUCTOR1_UDF_DEMO.PUBLIC.GET_MARKET_BASKET(INPUT_ITEM_SK NUMBER(38))
...
AS
 'SELECT INPUT_ITEM_SK
       ...
       , COUNT(DISTINCT SS_TICKET_NUMBER) BASKETS
    FROM INSTRUCTOR1_UDF_DEMO.PUBLIC.SALES
    WHERE SS_TICKET_NUMBER IN (SELECT SS_TICKET_NUMBER FROM INSTRUCTOR1_UDF_DEMO.PUBLIC.SALES WHERE SS_ITEM_SK = INPUT_ITEM_SK)
GROUP BY SS_ITEM_SK
ORDER BY 3 DESC, 2';

SELECT * FROM TABLE(INSTRUCTOR1_UDF_DEMO.PUBLIC.GET_MARKET_BASKET(6139));
-- PROVIDER --
...
-- PROVIDER --
USE DATABASE INSTRUCTOR1_udf_demo;
CREATE SHARE IF NOT EXISTS INSTRUCTOR1_UDF_DEMO_SHARE;
GRANT USAGE ON DATABASE INSTRUCTOR1_UDF_DEMO TO SHARE INSTRUCTOR1_UDF_DEMO_SHARE;
GRANT USAGE ON SCHEMA INSTRUCTOR1_UDF_DEMO.PUBLIC to share INSTRUCTOR1_UDF_DEMO_SHARE;
GRANT USAGE ON FUNCTION INSTRUCTOR1_UDF_DEMO.PUBLIC.get_market_basket(number) to share INSTRUCTOR1_UDF_DEMO_SHARE;
ALTER SHARE INSTRUCTOR1_UDF_DEMO_SHARE ADD ACCOUNTS=[consumer-account];
-- PROVIDER --
...
-- CONSUMER --
USE ROLE TRAINING_ROLE;
CREATE DATABASE INSTRUCTOR1_UDF_TEST FROM SHARE [provider-account].INSTRUCTOR1_UDF_DEMO_SHARE;
GRANT IMPORTED PRIVILEGES ON DATABASE INSTRUCTOR1_UDF_TEST TO ROLE PUBLIC;
USE DATABASE INSTRUCTOR1_UDF_TEST;
SELECT * FROM TABLE(INSTRUCTOR1_UDF_TEST.PUBLIC.GET_MARKET_BASKET(6139));
-- CONSUMER --
...
-- PROVIDER --
DESCRIBE SHARE INSTRUCTOR1_UDF_DEMO_SHARE;
-- PROVIDER --
