...
CREATE WAREHOUSE IF NOT EXISTS INSTRUCTOR1_REPL_WH  
   WITH WAREHOUSE_SIZE = 'XSMALL' AUTO_SUSPEND = 300;
CREATE DATABASE IF NOT EXISTS INSTRUCTOR1_REPL_DB;
CREATE SCHEMA IF NOT EXISTS REPL_SCHEMA;
USE INSTRUCTOR1_REPL_DB.REPL_SCHEMA;
...
-- 19.3.4  Promote INSTRUCTOR1_REPL_DB to serve as a primary database.

-- PRIMARY --
ALTER DATABASE INSTRUCTOR1_REPL_DB ENABLE REPLICATION 
...
-- PRIMARY --
ALTER DATABASE INSTRUCTOR1_REPL_DB ENABLE FAILOVER 
    TO ACCOUNTS [SECONDARY-REGION].[SECONDARY-ACCOUNT-LOCATOR];
-- PRIMARY --
...
-- SECONDARY --
CREATE DATABASE INSTRUCTOR1_REPL_DB AS REPLICA 
    OF [PRIMARY-REGION].[PRIMARY-ACCOUNT-LOCATOR].INSTRUCTOR1_REPL_DB;
...
-- SECONDARY --
GRANT OWNERSHIP ON DATABASE INSTRUCTOR1_REPL_DB TO ROLE TRAINING_ROLE;
GRANT OWNERSHIP ON SCHEMA INSTRUCTOR1_REPL_DB.PUBLIC TO ROLE TRAINING_ROLE;
-- SECONDARY --
...
-- SECONDARY --
USE ROLE TRAINING_ROLE;
ALTER DATABASE INSTRUCTOR1_REPL_DB REFRESH;
-- SECONDARY --
...
CREATE WAREHOUSE IF NOT EXISTS INSTRUCTOR1_REPL_WH 
    WITH WAREHOUSE_SIZE = 'XSMALL' AUTO_SUSPEND = 300;
USE DATABASE INSTRUCTOR1_REPL_DB;
...
-- SECONDARY --
USE WAREHOUSE INSTRUCTOR1_REPL_WH;
...
SELECT 
    ...
    DETAILS
FROM TABLE(INFORMATION_SCHEMA.DATABASE_REFRESH_PROGRESS('INSTRUCTOR1_REPL_DB')); 
...
FROM TABLE (flatten(input=>parse_json(
    SYSTEM$database_refresh_progress('INSTRUCTOR1_REPL_DB'))));
-- SECONDARY --
...
-- SECONDARY --
SELECT *
FROM TABLE(INFORMATION_SCHEMA.DATABASE_REFRESH_HISTORY('INSTRUCTOR1_REPL_DB')); 
-- SECONDARY --
...
CREATE OR REPLACE TASK INSTRUCTOR1_REPL_DB_REFRESH_TASK  
    WAREHOUSE = INSTRUCTOR1_REPL_WH 
    SCHEDULE = '1 MINUTE'
AS ALTER DATABASE INSTRUCTOR1_REPL_DB REFRESH;
-- SECONDARY --
...
ALTER TASK INSTRUCTOR1_REPL_DB_REFRESH_TASK RESUME;
...
SELECT * FROM TABLE(INFORMATION_SCHEMA.TASK_HISTORY()) 
  WHERE DATABASE_NAME LIKE UPPER('INSTRUCTOR1_DB');
...
ALTER TASK INSTRUCTOR1_REPL_DB_REFRESH_TASK SUSPEND;
...
ALTER DATABASE INSTRUCTOR1_REPL_DB PRIMARY;
-- SECONDARY --
...
-- PRIMARY --
USE ROLE TRAINING_ROLE;
DROP DATABASE INSTRUCTOR1_REPL_DB;
DROP WAREHOUSE INSTRUCTOR1_REPL_WH;
-- PRIMARY --
...
USE ROLE TRAINING_ROLE;
DROP DATABASE INSTRUCTOR1_REPL_DB;
DROP SCHEMA INSTRUCTOR1_db.TASKS; 
DROP WAREHOUSE INSTRUCTOR1_REPL_WH;
-- SECONDARY --
