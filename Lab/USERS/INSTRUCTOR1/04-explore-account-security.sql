...
CREATE NETWORK POLICY INSTRUCTOR1_policy ALLOWED_IP_LIST=('12.13.14.15');


-- 4.1.3   Describe the policy to verify that it was set correctly.

DESCRIBE NETWORK POLICY INSTRUCTOR1_policy;


-- 4.1.4   Now, create a new user:

CREATE USER INSTRUCTOR1_testuser PASSWORD = 'Password@1' MUST_CHANGE_PASSWORD = FALSE;


-- 4.1.5   Log out of the Snowflake account, and log back in as the user
--         INSTRUCTOR1_testuser you just created. This is just to verify that the
--         new user can log in. After you have done that, log back out and log
--         in as yourself INSTRUCTOR1.

-- 4.1.6   Now apply the network policy you created earlier to your test user:

ALTER USER INSTRUCTOR1_testuser SET NETWORK_POLICY = INSTRUCTOR1_policy;


-- 4.1.7   Log out of the Snowflake account, and try to log in as your test user
--         INSTRUCTOR1_testuser. You will not be able to. Then log in as yourself
--         INSTRUCTOR1. Since the network policy has only been applied to the test
--         user, it does not impact your ability to access the Snowflake
--         account.

-- 4.1.8   Drop the policy and user you created:

DROP USER INSTRUCTOR1_testuser;
DROP NETWORK POLICY INSTRUCTOR1_policy;
...
ALTER SCHEMA INSTRUCTOR1_db.public SET DATA_RETENTION_TIME_IN_DAYS = 10;
...
ALTER DATABASE INSTRUCTOR1_db SET DATA_RETENTION_TIME_IN_DAYS = 2;
...
ALTER SCHEMA INSTRUCTOR1_db.public UNSET DATA_RETENTION_TIME_IN_DAYS;
...
DROP TABLE tt_set30;
ALTER DATABASE INSTRUCTOR1_db UNSET DATA_RETENTION_TIME_IN_DAYS;
