...
CREATE TEMPORARY TABLE INSTRUCTOR1_db.public.lineitem_training 
  AS SELECT * FROM  lineitem SAMPLE (90);

CREATE TEMPORARY TABLE INSTRUCTOR1_db.public.lineitem_testing
  AS SELECT * FROM lineitem
      WHERE l_orderkey NOT IN (SELECT l_orderkey
  FROM INSTRUCTOR1_db.public.lineitem_training);
...
ALTER WAREHOUSE INSTRUCTOR1_query_wh
...
USE WAREHOUSE INSTRUCTOR1_QUERY_WH;
ALTER WAREHOUSE INSTRUCTOR1_QUERY_WH SET WAREHOUSE_SIZE = 'XSMALL';
...
USE ROLE training_role;
CREATE SCHEMA INSTRUCTOR1_db.hierarchy;
USE SCHEMA INSTRUCTOR1_db.hierarchy;
USE WAREHOUSE INSTRUCTOR1_query_wh;
