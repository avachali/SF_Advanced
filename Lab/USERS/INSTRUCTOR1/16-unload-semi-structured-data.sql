...
create or replace stage INSTRUCTOR1_unload;
...
--         JSON files in the INSTRUCTOR1_unload stage, with a max file size of 10MB.
...
copy into @INSTRUCTOR1_unload/json/weather/2019/US/stations from 
(
  select v from TRAINING_DB.WEATHER.ISD_2019_TOTAL
...
list @INSTRUCTOR1_unload/json/weather/2019/US/;
...
CREATE OR REPLACE PROCEDURE UNLOAD_ISD_2019_TOTAL(COUNTRY_CODE STRING)
    ...
    try {
      var unload_query = "copy into @INSTRUCTOR1_unload/json/weather/2019/" +  
                          COUNTRY_CODE + 
                         "/stations from" +
                         "( select v from TRAINING_DB.WEATHER.ISD_2019_TOTAL" +
...
copy into @INSTRUCTOR1_unload/json/customers from 
(
  select object_construct(
          'customer_info',object_construct(
...
copy into @INSTRUCTOR1_unload/parquet/weather/2019/US/stations from 
(
  SELECT weather.t as date, 
       v:station.name::STRING AS station,
       v:station.country::STRING AS country,
...
list @INSTRUCTOR1_unload/parquet/weather/2019/US/stations;


-- 16.4.6  Query the PARQUET file in the stage.

select $1 from @INSTRUCTOR1_unload/parquet/weather/2019/US/stations_0_0_0.snappy.parquet (file_format => parquetformat);
