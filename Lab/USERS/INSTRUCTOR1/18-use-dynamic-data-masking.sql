...
CREATE OR REPLACE SCHEMA INSTRUCTOR1_db.mask_lab;
USE SCHEMA INSTRUCTOR1_db.mask_lab;
...
USE ROLE SYSADMIN;
CREATE OR REPLACE MASKING POLICY INSTRUCTOR1_db.mask_lab.name_mask AS
...
CREATE OR REPLACE MASKING POLICY INSTRUCTOR1_db.mask_lab.email_mask AS(val STRING) RETURNS STRING ->
  CASE
    WHEN current_role() IN ('TRAINING_ROLE') THEN val
...
CREATE OR REPLACE MASKING POLICY INSTRUCTOR1_db.mask_lab.age_mask AS(val INTEGER) RETURNS INTEGER ->
  CASE
    WHEN current_role() IN ('TRAINING_ROLE') THEN val
      ELSE null
  END;
...
GRANT USAGE ON DATABASE INSTRUCTOR1_db TO ROLE PUBLIC;
GRANT USAGE ON SCHEMA INSTRUCTOR1_db.mask_lab TO ROLE PUBLIC;
GRANT SELECT ON ALL TABLES IN SCHEMA INSTRUCTOR1_db.mask_lab TO ROLE PUBLIC;
