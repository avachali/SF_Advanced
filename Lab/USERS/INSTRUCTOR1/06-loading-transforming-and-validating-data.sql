...
--         First, click on your database INSTRUCTOR1_DB in the navigator. Locate and
...
CREATE  OR REPLACE TEMPORARY STAGE INSTRUCTOR1_STAGE;


-- 6.2.3   Change your warehouse size to small:

ALTER WAREHOUSE INSTRUCTOR1_load_wh SET WAREHOUSE_SIZE = SMALL;


-- 6.2.4   Download a file from the Citibike table to the stage you created:

COPY INTO @INSTRUCTOR1_STAGE/citibike/singlefile/citibike.tbl
...
ls @INSTRUCTOR1_STAGE/citibike/singlefile;
...
COPY INTO @INSTRUCTOR1_STAGE/citibike/multiplefiles/citibike_
  FROM (SELECT * FROM citibike.schema1.trips)
  FILE_FORMAT=(FORMAT_NAME=training_db.traininglab.MYPIPEFORMAT)
  SINGLE=FALSE;

ls @INSTRUCTOR1_STAGE/citibike/multiplefiles;
...
-- 6.3.7   Alter the INSTRUCTOR1_load_wh and increase its size:

ALTER WAREHOUSE INSTRUCTOR1_LOAD_WH SET WAREHOUSE_SIZE='X-LARGE';
...
COPY INTO INSTRUCTOR1_DB.PUBLIC.CUSTOMER 
...
--         INTO statements to load each table in your INSTRUCTOR1_DB.PUBLIC schema.
--         Remaining tables:
...
-- 6.4.1   Empty the REGION Table in the PUBLIC schema of your INSTRUCTOR1_DB:
...
CREATE FILE FORMAT INSTRUCTOR1_FIXED TYPE = 'CSV' 
                   COMPRESSION = 'AUTO' 
...
COPY INTO nation_tbl
  FROM (SELECT CAST(SUBSTR($1,1,2) AS NUMBER)
        ...
        FROM '@training_db.traininglab.ed_stage/coredata/TCPH/FIXFORMAT'
          ...
          FILE_FORMAT=(FORMAT_NAME=INSTRUCTOR1_FIXED);
...
-- 6.9.6   Select INSTRUCTOR1_DB Database and confirm that the REGION table has data
--         loaded.
